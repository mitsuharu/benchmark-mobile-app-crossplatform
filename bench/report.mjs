...
const TIMINGS = [
  ['coldStartMs', 'コールドスタート（プロセス開始 → ホーム画面）'],
  ['searchOpenColdMs', '検索画面の表示（初回）'],
  ['searchOpenWarmMs', '検索画面の表示（2 回目）'],
  ['searchRenderMs', '検索 → 結果の描画'],
  ['searchToHomeMs', '検索 → ホーム画面が結果を受信'],
  ['commandMs', 'ホーム画面 → 検索画面へのキーワード差し替え'],
]

const MEMORY = [
  ['homeIdle', 'ホーム画面の表示後'],
  ['searchOpened', '検索画面の表示後'],
  ['afterSearch', '検索後'],
  ['backToHome', 'ホーム画面に戻った後'],
]
