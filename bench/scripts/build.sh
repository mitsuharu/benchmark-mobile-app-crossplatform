...
case "$FRAMEWORK/$PLATFORM" in
  ...
  expo/ios)
    ...
    # `expo prebuild` names the project after the app, and the workspace holds
    # a scheme per pod, so the app's scheme is taken from the project's name
    # rather than from the workspace's scheme list.
    project="$(ls -d "$ROOT"/expo/app/ios/*.xcodeproj | head -1)"
