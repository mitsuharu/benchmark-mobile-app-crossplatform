 ...
 * the search screen opened twice, one search and one keyword command.
 */
export function computeTimings(markers) {
  // Markers can arrive out of order when one screen's timestamps reach the
  // log through another layer, so pair them by time.
  const sorted = [...markers].sort((a, b) => a.epochMs - b.epochMs)
  return {
    coldStartMs: between(sorted, 'processStart', 'homeFirstFrame'),
    searchOpenColdMs: between(
      sorted,
      'searchOpenTapped',
      'searchFirstFrame',
      0,
    ),
    searchOpenWarmMs: between(
      sorted,
      'searchOpenTapped',
      'searchFirstFrame',
      1,
    ...
    searchToHomeMs: between(sorted, 'searchTapped', 'resultsReceived'),
