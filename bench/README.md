...
- **レート制限**: 未認証の検索は 10 リクエスト/分までで、3 実装 × 2 プラットフォーム × 複数回の計測に足りない。
- **ばらつき**: 通信時間が結果の大半を占めてしまい、フレームワークの差（JSON の変換、描画）が見えなくなる。
...
[agent-device](https://github.com/callstack/agent-device) で端末を操作し、全実装を同じシナリオで計測します。
手順の全体は [run-benchmark スキル](../.claude/skills/run-benchmark/SKILL.md) にまとめています。
...
Android エミュレータの値はエミュレータを起動してからの時間で大きく変わるため、日をまたいだ結果は並べられません。
1 実装だけ測り直したときは、同じ表の他の実装も測り直します。

### 計測用ビルド

`scripts/build.sh` は、検索先をモックサーバに向けたビルドを `artifacts/<build>/<framework>/<platform>/` に作ります。
ツールチェーンごとに成果物の名前が違う（`App.app` / `Runner.app` / Expo のアプリ名）ので、
どれも `App.app`・`app-<build>.apk` という名前に揃えて置きます。

検索先の渡し方は実装ごとに違います。

| 実装 | 渡し方 |
| --- | --- |
| native / iOS | `BENCH_API_BASE_URL` のビルド設定 → Info.plist |
| native / Android | `-PbenchApiBaseUrl` → `BuildConfig` |
| Flutter | `--dart-define=BENCH_API_BASE_URL=...` → `String.fromEnvironment` |
| Expo | `EXPO_PUBLIC_BENCH_API_BASE_URL` → Metro がバンドル時に埋め込む |
...
(cd ../expo/app && EXPO_PUBLIC_BENCH_API_BASE_URL=http://127.0.0.1:8787 npx expo start) &
node run.mjs --platform ios --framework all --build debug  # → results/debug/
```

Expo のデバッグビルドは JS を Metro（8081）から読みます。`run.mjs` は Metro が動いていなければ止まり、
Android では `adb reverse tcp:8081 tcp:8081` も設定します。Metro を起動するときは、
計測用ビルドと同じ `EXPO_PUBLIC_BENCH_API_BASE_URL` を環境変数で渡してください（バンドル時に埋め込まれる値です）。
...
- **`--udid` はハードウェアの UDID。** `xcrun devicectl list devices` の Identifier ではなく、
...
2. ホーム画面が出たら、落ち着くのを待って（`--settle-ms`、既定 2 秒）メモリを取る
3. 検索画面を開く → メモリ
4. 「リポジトリを検索」→ 結果が出たらメモリ
5. キーワード `swift` のボタンを押す
6. 「ホームに戻る」→ メモリ
7. もう一度開いて戻る（2 回目の表示）
...
| 指標 | マーカー |
| --- | --- |
| コールドスタート | `processStart` → `homeFirstFrame` |
| 検索画面の表示（初回 / 2 回目） | `searchOpenTapped` → `searchFirstFrame` |
| 検索 → 描画 | `searchTapped` → `searchRendered` |
| 検索 → ホーム画面が受信 | `searchTapped` → `resultsReceived` |
...
- iOS の Flutter だけは、シミュレータでは Debug（JIT）でしか動きません。
  リリース（AOT）の値は実機の結果を見てください（[flutter/README.md](../flutter/README.md)）。
