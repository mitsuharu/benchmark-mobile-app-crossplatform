...
let presetKeywords = ["expo", "swift", "kotlin"]
...
final class RepoSearchModel: ObservableObject {
  ...
  init(keyword: String, client: GitHubClient, channel: AppChannel = .shared) {
...
struct RepoSearchView: View {
  ...
  init(
    keyword: String,
    apiBaseURL: URL = GitHubClient.defaultBaseURL,
    channel: AppChannel = .shared
  ) {
    _model = StateObject(
      wrappedValue: RepoSearchModel(
        keyword: keyword,
        client: GitHubClient(baseURL: apiBaseURL),
        channel: channel
      )
    )
    self.channel = channel
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      HStack(spacing: 8) {
        ForEach(presetKeywords, id: \.self) { preset in
          Button(preset) {
            ...
            channel.send(.setKeyword(preset))
          }
      ...
      HStack(spacing: 8) {
        ...
        ActionButton(label: "ホームに戻る", variant: .secondary) {
          dismiss()
        }
      }
      .padding(.horizontal, 20)
    ...
    .onAppear { BenchMarker.markAfterFrame("searchFirstFrame") }
