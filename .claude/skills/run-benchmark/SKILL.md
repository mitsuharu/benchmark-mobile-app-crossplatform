---
name: run-benchmark
description: Build the benchmark builds of the full-app implementations (native, flutter, expo) and measure cold start, response time, memory and app size on the iOS simulator / Android emulator / physical devices with agent-device, then update the results in README.md. Use when asked to run, re-run or update the benchmark.
---
...
- デバッグビルドも計測するときは、`./scripts/build.sh <framework> <platform> debug` で作り、
  Expo 用に Metro を起動してから（`cd expo/app && EXPO_PUBLIC_BENCH_API_BASE_URL=http://127.0.0.1:8787 npx expo start`）
  `--build debug` を付けて実行する。
...
- 1 回の計測は「コールド起動 → 検索画面を開く → 検索 → キーワード差し替え → 戻る → もう一度開く → 戻る」。
...
アプリ自体は `build.sh` が `SYMROOT` / `OBJROOT` を明示するので、この影響を受けません。
...
- iOS シミュレータの Flutter は Debug（JIT）でしか動かない。リリース（AOT）の値は実機の結果を使う。
