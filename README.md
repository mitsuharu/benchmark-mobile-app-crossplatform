...
- **検索 → ホーム画面が結果を受信** は native 6 ms・Flutter 5 ms に対し Expo 31 ms。
  Expo は JS のイベントバスを通り、FlatList の再描画と同じフレームに乗るぶん長くなります。
...
- **コールドスタートは 3 実装とも 234〜281 ms で差がありません**（native 281 ms、Flutter 265 ms、Expo 234 ms）。
  埋め込みの比較では Flutter が約 810 ms と突出していましたが、あれは「ネイティブのホスト画面を出すまでに
  Flutter エンジンを起動していた」ぶんでした。アプリ全体を Flutter で作ると、エンジンの起動と最初の画面の描画が
  1 つの流れになり、ネイティブと同程度に収まります。
...
- **アプリ全体をクロスプラットフォームで作る場合、起動時間はネイティブとほぼ変わりません。**
  埋め込み（brownfield）では「ネイティブのホスト画面を出すためにランタイムを先に起動する」ぶんが丸ごと乗っていましたが、
